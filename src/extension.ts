...
async function cache(): Promise<void> {
  try {
    ...
    channel.appendLine("Looking for parseable documents...");
    const uris: Uri[] = await Fetcher.findAllParseableDocuments();

    if (!uris || uris.length === 0) {
      channel.appendLine("Found no documents");
      notifier.statusBarItem.hide();
      return;
    }

    channel.appendLine("Found all parseable documents.");
    ...
    try {
      await Bluebird.map(
        ...
        async uri => {
          try {
            channel.appendLine("Parsing uri " + uri.path);
            ...
            channel.appendLine("Failed to parse uri " + uri.path);
            failedLogs += `${uri.path}\n`;
            failedLogsCount++;
          }
          filesParsed++;
    ...
    channel.appendLine(uris.length + " parseable documents found");
    channel.appendLine(definitions.length + " CSS class definitions found");
    channel.appendLine(
      uniqueDefinitions.length + " unique CSS class definitions found"
    );
    channel.appendLine(
      failedLogsCount + " failed attempts to parse. List of the documents:"
    );
